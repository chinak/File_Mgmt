...
/** 主流程状态合法转换表（8个状态值） */
export const MAIN_STATUS_TRANSITIONS: Record<MainStatus, Partial<Record<TransitionAction, MainStatus>>> = {
  'pending_shipment': {
...
/** 综合部归档状态合法转换表（4个状态值） */
export const ARCHIVE_STATUS_TRANSITIONS: Record<ArchiveSubStatus, Partial<Record<TransitionAction, ArchiveSubStatus>>> = {
  'archive_not_started': {
...
/** 操作-角色权限映射表 */
export const ACTION_ROLE_MAP: Record<TransitionAction, UserRole> = {
...
/** 操作对应的状态字段映射 */
const ACTION_STATUS_FIELD_MAP: Record<TransitionAction, 'status' | 'archive_status'> = {
...
export class StateMachineService {
  ...
  /** 主流程状态转换 */
  private transitionMainStatus(record: ArchiveRecord, action: TransitionAction): TransitionResult {
    ...
    const transitions = MAIN_STATUS_TRANSITIONS[currentStatus as MainStatus];
  ...
  /** 综合部归档状态转换 */
  private transitionArchiveStatus(record: ArchiveRecord, action: TransitionAction): TransitionResult {
    const currentStatus = record.archiveStatus;
    const transitions = ARCHIVE_STATUS_TRANSITIONS[currentStatus];
    const newStatus = transitions?.[action];
